...
def MI_loss_nat(i, model, x_natural, y, x_adv, local_n, global_n, epoch):
    ...
    if (i + 1) % args.print_freq == 0:
        ...
        print('Epoch [%d], Iter [%d/%d] Train target model. Natural MI: %.4f; Loss_ce: %.4f; Loss_all: %.4f'
              % (epoch, i + 1, 100000 // args.batch_size, loss_mi.item(), loss_ce.item(), loss_all.item()))
...
def MI_loss(i,model, x_natural, y, x_adv, local_n, global_n, local_a, global_a, epoch):
    ...
    if torch.nonzero(index).size(0) != 0:
        ...
        # manha
        #loss_mea_n=torch.norm(loss_n - loss_a, p=1)
        # chi_square
        #loss_mea_n=torch.abs(torch.sum((loss_n - loss_a)**2 / (loss_n + loss_a + 1e-8)))
        # bhattacharyya
        #loss_mea_n=-torch.log(torch.sum(torch.sqrt(loss_n * loss_a)))
        #hellinger
        #loss_mea_n=torch.norm(torch.sqrt(loss_n) - torch.sqrt(loss_a), p=2) / torch.sqrt(torch.tensor(2.0).cuda())
        #correlation
        #loss_mea_n=1 - torch.dot(loss_n - loss_n.mean(), loss_a - loss_a.mean()) / (torch.norm(loss_n - loss_n.mean()) * torch.norm(loss_a - loss_a.mean()) + 1e-8)
        ...
        #loss_a_all =torch.abs(torch.tensor(1.0).cuda()-(loss_a_all - loss_a))
        #cosin
        loss_mea_a = torch.abs(torch.tensor(1.0).cuda() - torch.cosine_similarity(loss_n, loss_a, dim=0))
        #manha
        #loss_mea_a=torch.norm(loss_n - loss_a,p=1)
        #chi_square
        #loss_mea_a=torch.abs(torch.sum((loss_n-loss_a)**2/(loss_n+loss_a+1e-8)))

        # bhattacharyya
        #loss_mea_a=-torch.log(torch.sum(torch.sqrt(loss_n * loss_a)))
        #hellinger
        #loss_mea_a=torch.norm(torch.sqrt(loss_n) - torch.sqrt(loss_a), p=2) / torch.sqrt(torch.tensor(2.0).cuda())
        #correlation
        #loss_mea_a=1 - torch.dot(loss_n - loss_n.mean(), loss_a - loss_a.mean()) / (torch.norm(loss_n - loss_n.mean()) * torch.norm(loss_a - loss_a.mean()) + 1e-8)

        #cosine,corre
        loss_mi = 1.0*loss_mea_n + 1.0*loss_mea_a # +1.0* loss_a_all
        #manha
        #loss_mi=torch.abs(loss_mea_n-loss_mea_a)
    else:
        loss_mi = 0.0
...
def main():
    ...
    testset = CIFAR10(args.data_dir,train=False,download=False,transform=trans_test)
    ...
    '''
    trainset = TinyImageNetDataset(root="./", mode='train',download=False,transform=trans_train)
    train_loader = DataLoader(trainset,batch_size=args.batch_size,shuffle=True,drop_last=True,pin_memory=torch.cuda.is_available())
    print("训练样本个数：",len(trainset))
    ...
    local_n = local_n.cuda()
    global_n = global_n.cuda()
    local_a = local_a.cuda()
    global_a = global_a.cuda()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            adv = craft_adversarial_example_pgd(model=target_model, x_natural=data, y=target, step_size=0.125,
                                                epsilon=0.5, perturb_steps=40, distance='l_2')
