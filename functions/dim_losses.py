...
def fenchel_dual_loss(l, m, measure=None):
    ...
    Args:
        l: Local feature map.没编码前的特征图，但其实也经历了和编码结构一样的压缩
        m: Multiple globals feature map.编码后的特征向量，目标就是使正例中的两者互信息最大
    ...
    m = m.permute(0, 2, 1)
    m = m.reshape(-1, units)#将通道数拎出来为了相乘

    # Outer product, we want a N x N x n_local x n_multi tensor.
    u = torch.mm(m, l.t())
    u = u.reshape(N, n_multis, N, n_locals).permute(0, 2, 3, 1)#就是变成 N*N 的形式，对角线为正例对

    # Since we have a big tensor with both positive and negative samples, we need to mask.
    mask = torch.eye(N).to(l.device)#形成正例对损失的 mask，为对角线
    n_mask = 1 - mask#剩下都是负例对
...
def copula_entropy_loss(l,m):
    '''
    input:
        l: Local feature map.没编码前的特征图，但其实也经历了和编码结构一样的压缩
        m: Multiple globals feature map.编码后的特征向量，目标就是使正例中的两者互信息最大
    output:
        loss:estimated entropy
    ...
    # Since we have a big tensor with both positive and negative samples, we need to mask.
    mask = torch.eye(N).to(l.device).unsqueeze(2)#形成正例对损失的 mask，为对角线
    ...
    pos_prob=torch.mean(u*mask)
    neg_prob=torch.mean(u*n_mask)

    pos_copula_entropy=-pos_prob*torch.log2(pos_prob)
    neg_copula_entropy=-neg_prob*torch.log2(neg_prob)

    pos_score=pos_copula_entropy/(pos_copula_entropy+neg_copula_entropy)
    neg_score=neg_copula_entropy/(pos_copula_entropy+neg_copula_entropy)
    '''
    #pos_weight=(u*mask).sum((1))/u.sum((1))
    #neg_weight=(u*n_mask).sum((1))/u.sum((1))

    #pos_score=torch.full(pos_weight.size(), pos_score)
    #neg_score=torch.full(neg_weight.size(), neg_score)
    #pos_score=pos_score.to(l.device)
    #neg_score=neg_score.to(l.device)
    p=(u*mask).sum((1))/mask.sum((1))
    q=(u*n_mask).sum((1))/n_mask.sum((1))
    r=torch.cat([p,q],dim=0)
    n=r.shape[0]
    cdf=torch.argsort(torch.argsort(r,dim=0),dim=0)/n
    u=cdf[:p.shape[0],:]
    v=cdf[p.shape[0]:,:]
    uv=torch.cat([u,v],dim=1)

    loss=-torch.sum(torch.pow(uv,1)*torch.log(uv+1e-20),dim=1)

    #print(cdf)


    #loss=(neg_score*neg_weight-pos_weight*pos_score).mean(dim=1)
